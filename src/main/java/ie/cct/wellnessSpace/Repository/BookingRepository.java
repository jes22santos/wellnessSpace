...
public interface BookingRepository extends JpaRepository<Bookings, Integer> {
    ...
    @Query(value = "SELECT * from bookings  where id_staff=?1", nativeQuery = true)
    List<Bookings> findAllByStaff(Integer id_staff);

    @Query(value = "SELECT * from bookings  where id_staff=?1 AND date BETWEEN ?2 AND ?3", nativeQuery = true)
